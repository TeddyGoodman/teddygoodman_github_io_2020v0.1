---
layout:     post
title:      "Python实现动态生成类的http接口-pingan动态监控项"
subtitle:   "python"
iframe:     "//teddygoodman.github.io/2020-3-24-Python-dynamic-map.md"
date:       2020-03-24
author:     "Teddy"
...
* 文件路经 `cloudeye/cloudeye-server/cloudeye/api/metric_api/dynamic_router_api.py`
...
from cloudeye.core.handlers import get_mapping, BaseHandler
from cloudeye.pkg.utils.logger import LOG
from cloudeye.core.metric.dynamic.dynamic import DynamicMetricDataHandler
from cloudeye.core.handlers.agent_error_handler import MetricErrorHandler
from cloudeye.db import api
...
class RouteRegister(object):
    ...
    def registerAPI():
        ...
        for raw in raw_tasks:
            if raw and raw.get('argus_upload'): #
...
| desc        | varchar             | YES  | 	 | 描述								         |
| argus_upload| bool                | NO   |     | 是否将数据发送到 argus 平台			         |
...
from cloudeye.core.handlers.device_info_handler import DeviceInfo
from cloudeye.pkg.utils.import_util import LOG
from cloudeye.core.model.cloudeye_data_model import MonitorItemDataModel
from cloudeye.core.model import cloudeye_data_model as DModle
from cloudeye.core.metric.dynamic.dynamic_rules import ValueRules
from cloudeye.core.metric.dynamic.dynamic_rules import TagRules
from cloudeye.core.metric.dynamic.dynamic_rules import OperationRules
from cloudeye.core.metric.publisher import Falcon
...
class DynamicMetricDataHandler(object):
    ...
    def __init__(self, monitor_item=None):
        ...
        self._argus_upload = mon_object.argus_upload  # @是否上报Argus
    ...
    def metrics_sender(self, value, tags):
        ...
        :return:
        """
        __argus_metric_type = Metric_Type[self._metric_type]
        ...
        try:
            m = {
                ...
                "step": self.step,
                "value": value,
                "counterType": __argus_metric_type,
                "tags": tags
            }
            # LOG.info("[DEBUG Send] %s" % m)
            Falcon.send([m])
        except Exception as e:
            LOG.exception("parser argus metric failed, e=%s" % e)
        return True
